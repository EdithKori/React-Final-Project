// src/pages/FlightBookingPage.jsx → FINAL + BOOKING MODAL
...
function FlightBookingPage() {
  ...
  const confirmBooking = () => {
    alert(`Flight ${selectedFlight.flight.iata} booked successfully!`);
    setShowModal(false);
      ...
      <div className="max-w-7xl mx-auto">
            ...
            {flights.map((flight, i) => (
              ...
              >
                ...
                <button
                  ...
                  className="mt-4 w-full bg-green-600 text-white px-4 py-2.5 rounded-lg hover:bg-green-700 transition font-medium text-sm sm:text-base shadow-md"
                >
                  Book Now
                </button>
              </div>
        ...
        {showModal && selectedFlight && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70">
            <div className="bg-gradient-to-br from-[#1e293b] to-[#334155] p-6 sm:p-8 rounded-2xl shadow-2xl max-w-md w-full border border-white/20">
              <h2 className="text-2xl sm:text-3xl font-bold text-green-400 mb-4">Confirm Booking</h2>
              <div className="space-y-2 text-sm sm:text-base text-gray-200 mb-6">
                <p><strong>Flight:</strong> {selectedFlight.flight.iata}</p>
                <p><strong>Airline:</strong> {selectedFlight.airline.name}</p>
                <p><strong>From:</strong> {selectedFlight.departure.airport}</p>
                <p><strong>Departure:</strong> {new Date(selectedFlight.departure.scheduled).toLocaleString()}</p>
                <p><strong>Destination:</strong> {cityName}</p>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-gray-600 text-white py-2.5 rounded-lg hover:bg-gray-700 transition font-medium"
                >
                  Cancel
                </button>
              </div>
